...
DESCRIPTION
    ...
    This card is a very confusing card. I hope to write my driver "cleaner"
    than the ones before it and thus make it a much better reference for
    people out there.

    RealTek (http://www.realtek.com.tw/) has been great and released Engrish
    documentation about their card. However, the documentation itself is
    generally crap - or at least is only good as a register reference.

    Andrew has contributed with great reference code and even better
    personal support in #dcdev@EFNet. Give the guy a damn hand.
...
/* All of the following code is *very* magic */
...
#define ENABLE_RTL_INTS
#ifdef ENABLE_RTL_INTS
    {
        ...
        entry.code = EXP_CODE_INT13;    /* No one uses IRQ 13 - at least, VOOT doesn't. */
        entry.mask1 = ASIC_MASK1_PCI;   /* Link on the ASIC/G2 PCI interrupt. */
        entry.handler = rtl_handler;

        /* STAGE: Add hook to interrupt table */
        rtl_info.hdl_tbl_index = add_exception_handler(exp_table, &(entry));
