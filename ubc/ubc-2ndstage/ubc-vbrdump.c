...
void ubc_break_work(void)
{
    ...
    /* About that VBR... */
    ubc_serial_write_str("[UBC] VBR == 0x");
    uint_to_string(vbr(), buffer);
    ubc_serial_write_str(buffer);
    ubc_serial_write_str("\r\n");

    /* Also, where is the stack? */
    ubc_serial_write_str("[UBC] R15 == 0x");
    uint_to_string(r15(), buffer);
    ubc_serial_write_str(buffer);
    ubc_serial_write_str("\r\n");
}
